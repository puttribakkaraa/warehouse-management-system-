            ...
            <form action="{{ route('incoming-goods.update', $incomingGood) }}" method="POST">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="item_id" class="form-select @error('item_id') is-invalid @enderror" required>
                            ...
                            @foreach($items as $item)
                                <option value="{{ $item->id }}" {{ old('item_id', $incomingGood->item_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }} ({{ $item->unit }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror" 
                               value="{{ old('quantity', $incomingGood->quantity) }}" min="0.01" step="0.01" required id="quantity">
                        @error('quantity')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="unit_price" class="form-control @error('unit_price') is-invalid @enderror" 
                               value="{{ old('unit_price', $incomingGood->unit_price) }}" min="0" step="1" required id="unitPrice">
                        @error('unit_price')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" 
                               value="{{ old('date', $incomingGood->date->format('Y-m-d')) }}" required>
                        @error('date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" 
                              rows="2">{{ old('notes', $incomingGood->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
