...
@section('content')
    <div class="card">
        <div class="card-header">
            <i class="bi bi-pencil me-2"></i>Edit Menu: {{ $menu->name }}
        </div>
        <div class="card-body">
                ...
                <div id="ingredientsContainer">
                    @foreach($menu->ingredients as $index => $ingredient)
                        <div class="ingredient-row row mb-2">
                            <div class="col-md-5">
                                <select name="ingredients[{{ $index }}][item_id]" class="form-select" required>
                            ...
                            <div class="col-md-4">
                                <input type="number" name="ingredients[{{ $index }}][quantity_required]" class="form-control" 
                                       value="{{ $ingredient->pivot->quantity_required }}" step="0.01" min="0.01" required>
                            </div>
                            <div class="col-md-3">
                                <button type="button" class="btn btn-danger btn-remove-ingredient" {{ $loop->count == 1 ? 'disabled' : '' }}>
...
@push('scripts')
<script>
    let ingredientIndex = {{ $menu->ingredients->count() }};
